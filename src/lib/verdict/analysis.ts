import { z } from "zod";
import type { PartySide } from "@/types";
import { APPEAL_OUTCOME_VALUES } from "@/types";
...
export const verdictAnalysisSchema = z.object({
  ...
  // Optional so verdicts from older agents still parse; engine falls back to a
  // margin-derived estimate when the judge omits it.
  shame_score: z.number().min(0).max(100).optional(),
});

export type VerdictAnalysis = z.infer<typeof verdictAnalysisSchema>;

// "Calculated based on logic": logic counts double when weighing the sides.
...
export function resolveWinnerSide(analysis: VerdictAnalysis): PartySide {
  ...
  if (a === b) return analysis.winner_side; // judge's discretion breaks exact ties
  return a > b ? "A" : "B";
}

/**
 * Shame fallback when the judge omits shame_score: scale the weighted margin
 * (0-30) onto 0-100 with a floor — every closed case carries some disgrace.
 */
